...
func TestServerClientGetByNumericName(t *testing.T) {
	env := newTestEnv()
	defer env.Teardown()

	env.Mux.HandleFunc("/servers/123", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusNotFound)
		json.NewEncoder(w).Encode(schema.ErrorResponse{
			Error: schema.Error{
	...
	server, _, err := env.Client.Server.Get(ctx, "123")
	if err != nil {
		t.Fatal(err)
	}
	if server == nil {
...
func TestServerGetMetrics(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedErr: "invalid argument 'server' [*hcloud.Server]: empty value '<nil>'",
		},
	}
